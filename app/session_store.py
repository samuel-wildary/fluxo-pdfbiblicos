...
class SessionStore:
    ...
    def add_message_to_history(self, chat_id: str, role: str, content: Any) -> None:
        key = f"history:{chat_id}"
    ...
    def set_agent_state(self, chat_id: str, state: dict[str, Any]) -> None:
        key = f"agent_state:{chat_id}"
        existing = self.get_agent_state(chat_id)
        existing.update(state)
        self.redis.setex(key, SESSION_TTL_SECONDS, json.dumps(existing))

    def clear_agent_state(self, chat_id: str) -> None:
        self.redis.delete(f"agent_state:{chat_id}")
